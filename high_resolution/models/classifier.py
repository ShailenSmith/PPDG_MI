...
def forward_hook(module, inputs, output):
    ...
    # input 是一个包含输入数据的元组
    input_features_of_last_layer = inputs[0].detach()
...
class Classifier(BaseModel):
    ...
    def _build_model(self, architecture, pretrained):
        ...
        # Note: inception_v3 expects input tensors with a size of N x 3 x 299 x 299, aux_logits are used per default
            ...
            # model_path = "/home/comp/23481501/code/Plug-and-Play-Attacks/results/inceptionv3_celeba/Classifier_0.9487.pth"
            # state_dict = torch.load(model_path)['model_state_dict']
            # new_state_dict = {k.replace('model.', ''): v for k, v in state_dict.items() if not k.startswith('model.fc')}
    ...
    def eval_fc_weights_similarity(self,
                                   evaluation_data,
                                   batch_size=128,
                                   dataloader_num_workers=4):
        evalloader = DataLoader(evaluation_data,
        ...
        model_path = "./results/resnet18_20231219_222422/Classifier_0.9128.pth"
        state_dict = torch.load(model_path)['model_state_dict']
        new_state_dict = {k.replace('model.', ''): v for k, v in state_dict.items()}
        # new_state_dict = {k.replace('model.', '_orig_mod.'): v for k, v in state_dict.items()}
        tmp = self.model.load_state_dict(new_state_dict)
        self.eval()

        cosine_sim_sums = defaultdict(float)
        l2_dist_sums = defaultdict(float)
        ...
        with torch.no_grad():
            for inputs, labels in tqdm(evalloader,
                ...
                for i in range(inputs.shape[0]):
                    ...
                    input_feature = input_features_of_last_layer[i]  # 获取输入特征
                    class_weight = self.model.fc.weight[label]

                    # 计算余弦相似度
                    cosine_similarity = F.cosine_similarity(input_feature.unsqueeze(0),
                                                            class_weight.unsqueeze(0)).item()
                    cosine_sim_sums[label] += cosine_similarity

                    # 计算 L2 距离
                    l2_distance = torch.norm(class_weight - input_feature).item()
                    l2_dist_sums[label] += l2_distance

                    # 扩展输入特征和权重
                    # extended_input_feature = torch.cat([input_feature, torch.tensor([1.0], device=self.device)])
                    # class_weight_bias = torch.cat([self.model.fc.weight[label], self.model.fc.bias[label].unsqueeze(0)])
                    #
                    # # 计算余弦相似度
                    # cosine_similarity = F.cosine_similarity(extended_input_feature.unsqueeze(0),
                    #                                         class_weight_bias.unsqueeze(0)).item()
                    # cosine_sim_sums[label] += cosine_similarity
                    #
                    # # 计算 L2 距离
                    # l2_distance = torch.norm(class_weight_bias - extended_input_feature).item()
                    # l2_dist_sums[label] += l2_distance

                    counts[label] += 1

            # 计算每个类别的余弦相似度和 L2 距离的均值
            average_cosine_similarities = {label: cosine_sim_sums[label] / counts[label] for label in counts}
            average_l2_distances = {label: l2_dist_sums[label] / counts[label] for label in counts}
            ...
            for label in sorted_labels:
                print(
                    f"Class {label}: Average Cosine Similarity = {average_cosine_similarities[label]}, "
                    f"Average L2 Distance = {average_l2_distances[label]}"
                )
