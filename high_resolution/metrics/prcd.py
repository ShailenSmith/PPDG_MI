...
#
# class PRCD:
#     def __init__(self, dataset_real, dataset_fake, device, crop_size=None, generator=None, batch_size=128, dims=2048,
#                  num_workers=16, gpu_devices=[]):
...
#
#     def compute_metric(self, target_classes, k=3, target_id=None, rtpt=None):
#         precision_list = []
#         recall_list = []
#         density_list = []
#         coverage_list = []
#         for step, cls in enumerate(target_classes):
...
#                 sphere_counter = (distances_fake_to_real <= radius_real.repeat(num_samples, 1)).float().sum(
#                     dim=0).mean()
...
#                 if rtpt:
#                     rtpt.step(
#                         subtitle=f'PRCD Computation step {step} of {len(target_classes)}')
#
